      ...
      <div class="videopost-wrapper">
        ...
        <form class="postForm">
          ...
          <button ref="postButton" class="postButton" @click.prevent="postVideo">
            <span v-show="!isSubmitted">发布视频</span>
            <span v-show="isSubmitted">发布中...</span>
          </button>
        </form>
      </div>
    </div>
...
export default {
  ...
  methods: {
    postVideo() {
      if (
        this.postData.playUrl.trim() &&
        this.postData.title.trim() &&
        this.postData.coverSrc.trim()
      ) {
        const postButton = this.$refs.postButton;
