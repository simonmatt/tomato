...
<div class="pagination">
  <ul class="page-list">
    ...
    <li v-for="index of all" @click="indexPage(index)" v-bind:key="index">
      <router-link v-bind:to="`/videolist?p=${index}`" :class="{checked:current===index}">{{index}}</router-link>
    </li>
...
export default {
  ...
  methods: {
    pre() {
      ...
      this.$router.push({
        path: "videolist",
        query: { p: this.$store.state.current }
      });
